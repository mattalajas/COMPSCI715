...
# Define a new class for ConvFormer-LSTM model
class ConvFormer_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(ConvFormer_LSTM_Model, self).__init__()

        # Extract frame features
        self.convformer = pre_trained_model
        num_convformer_features = self.convformer.head.fc.fc2.in_features  # Assuming final layer is in this format
        self.convformer.head.fc.fc2 = nn.Identity()  # Remove final FC layer to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_convformer_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through ConvFormer
        convformer_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.convformer(frame)  # Pass each frame through ConvFormer
            convformer_features.append(features)

        # Stack the features along the time dimension
        convformer_features = torch.stack(convformer_features, dim=1)  # Shape: (batch_size, num_frames, num_convformer_features)
        ...
        lstm_out, (hn, cn) = self.lstm(convformer_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
# Load pre-trained ConvFormer model from timm
pre_trained_model = timm.create_model('convformer_b36', pretrained=True)

# Initialize ConvFormer-LSTM model
model = ConvFormer_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=frames)
