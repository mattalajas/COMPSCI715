...
class BEiT_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(BEiT_LSTM_Model, self).__init__()

        # Extract frame features using BEiT
        self.beit = pre_trained_model
        num_beit_features = self.beit.head.in_features
        self.beit.head = nn.Identity()  # Remove final classification head to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_beit_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through BEiT
        beit_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.beit(frame)  # Pass each frame through BEiT
            beit_features.append(features)

        # Stack the features along the time dimension
        beit_features = torch.stack(beit_features, dim=1)  # Shape: (batch_size, num_frames, num_beit_features)
        ...
        lstm_out, (hn, cn) = self.lstm(beit_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
val_loader = DataLoader(val_set, batch_size=32, shuffle=True, num_workers=8, pin_memory=True)

# Set device (GPU if available, else CPU)
device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")

# Initialize the model
pre_trained_model = timm.create_model('beit_base_patch16_224', pretrained=True)
model = BEiT_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
...
# Initialize TensorBoard writerbatc
