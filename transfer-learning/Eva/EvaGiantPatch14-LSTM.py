...
class EVA_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(EVA_LSTM_Model, self).__init__()

        # Extract frame features using EVA_Giant
        self.eva = pre_trained_model
        num_eva_features = self.eva.head.in_features
        self.eva.head = nn.Identity()  # Remove final FC layer to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_eva_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through EVA
        eva_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.eva(frame)  # Pass each frame through EVA
            eva_features.append(features)

        # Stack the features along the time dimension
        eva_features = torch.stack(eva_features, dim=1)  # Shape: (batch_size, num_frames, num_eva_features)
        ...
        lstm_out, (hn, cn) = self.lstm(eva_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
pre_trained_model = timm.create_model('eva_giant_patch14_224', pretrained=True)
model = EVA_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
