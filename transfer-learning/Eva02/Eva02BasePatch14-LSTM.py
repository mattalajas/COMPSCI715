...
class EVA02_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(EVA02_LSTM_Model, self).__init__()

        # Extract frame features using EVA02
        self.eva02 = pre_trained_model
        num_eva02_features = self.eva02.embed_dim  # EVA02 uses embed_dim for feature dimensions
        self.eva02.head = nn.Identity()  # Remove classification head to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_eva02_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through EVA02
        eva02_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.eva02(frame)  # Pass each frame through EVA02
            eva02_features.append(features)

        # Stack the features along the time dimension
        eva02_features = torch.stack(eva02_features, dim=1)  # Shape: (batch_size, num_frames, num_eva02_features)
        ...
        lstm_out, (hn, cn) = self.lstm(eva02_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
train_loader = DataLoader(train_set, batch_size=24, shuffle=True, num_workers=8, pin_memory=True)
val_loader = DataLoader(val_set, batch_size=24, shuffle=True, num_workers=8, pin_memory=True)

# Set device (GPU if available, else CPU)
device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")

# Initialize the EVA02 model
pre_trained_model = timm.create_model('eva02_base_patch14_224', pretrained=True)
model = EVA02_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
