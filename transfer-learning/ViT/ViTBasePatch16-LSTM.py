...
class ViT_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        ...
        # Extract frame features using the ViT model
        self.vit = pre_trained_model
        num_vit_features = self.vit.head.in_features  # In ViT, the final layer is called 'head'
        self.vit.head = nn.Identity()  # Remove final classifier layer to get the feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_vit_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            features = self.vit(frame)  # Pass each frame through ViT
            vit_features.append(features)

        # Stack the features along the time dimension
        vit_features = torch.stack(vit_features, dim=1)  # Shape: (batch_size, num_frames, num_vit_features)
        ...
        lstm_out, (hn, cn) = self.lstm(vit_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
model = ViT_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
