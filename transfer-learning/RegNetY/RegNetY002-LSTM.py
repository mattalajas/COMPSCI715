...
class RegNetY_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(RegNetY_LSTM_Model, self).__init__()

        # Extract frame features using RegNetY
        self.regnety = pre_trained_model
        num_regnet_features = self.regnety.head.fc.in_features
        self.regnety.head.fc = nn.Identity()  # Remove final FC layer to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_regnet_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through RegNetY
        regnet_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.regnety(frame)  # Pass each frame through RegNetY
            regnet_features.append(features)

        # Stack the features along the time dimension
        regnet_features = torch.stack(regnet_features, dim=1)  # Shape: (batch_size, num_frames, num_regnet_features)
        ...
        lstm_out, (hn, cn) = self.lstm(regnet_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
# Initialize the model with RegNetY-002
pre_trained_model = timm.create_model('regnety_002', pretrained=True)
model = RegNetY_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
