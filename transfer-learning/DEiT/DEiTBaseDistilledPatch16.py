...
model.head = nn.Linear(num_features, 1000)  # Changed output dimension to 1000

# Add a custom regression layer
model.regression_head = nn.Linear(1000, len(cols_to_predict_value))  

# Move model to the device (GPU)
model = model.to(device)

# Define forward function to include regression layer
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        x = self.model(x)
        x = self.model.regression_head(x)
        return x
