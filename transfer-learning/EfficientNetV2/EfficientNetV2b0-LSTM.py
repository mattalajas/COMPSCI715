...
class EfficientNet_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(EfficientNet_LSTM_Model, self).__init__()

        # Extract frame features using EfficientNet
        self.efficientnet = pre_trained_model
        num_efficientnet_features = self.efficientnet.classifier.in_features
        self.efficientnet.classifier = nn.Identity()  # Remove final classifier layer to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_efficientnet_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        for t in range(seq_len):
            ...
            features = self.efficientnet(frame)  # Pass each frame through EfficientNet
            efficientnet_features.append(features)

        # Stack the features along the time dimension
        efficientnet_features = torch.stack(efficientnet_features, dim=1)  # Shape: (batch_size, num_frames, num_efficientnet_features)
        ...
        lstm_out, (hn, cn) = self.lstm(efficientnet_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
# Initialize the model with EfficientNetB0
pre_trained_model = timm.create_model('tf_efficientnetv2_b0', pretrained=True)
model = EfficientNet_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
