...
class VGG16_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(VGG16_LSTM_Model, self).__init__()

        # Extract frame features
        self.vgg16 = pre_trained_model
        # Remove the classifier (final FC layers) to get feature vector
        self.vgg16.classifier = nn.Identity()
        num_vgg_features = 512 * 7 * 7  # VGG16 feature map size after last conv layer

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_vgg_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through VGG16
        vgg_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.vgg16(frame)  # Pass each frame through VGG16
            vgg_features.append(features.view(batch_size, -1))  # Flatten the feature maps

        # Stack the features along the time dimension
        vgg_features = torch.stack(vgg_features, dim=1)  # Shape: (batch_size, num_frames, num_vgg_features)
        ...
        lstm_out, (hn, cn) = self.lstm(vgg_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
pre_trained_model = torchvision.models.vgg16(pretrained=True)
model = VGG16_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
