...
class ConvNeXt_LSTM_Model(nn.Module):
    def __init__(self, pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=4, num_frames=12):
        super(ConvNeXt_LSTM_Model, self).__init__()

        # Extract frame features using ConvNeXt
        self.convnext = pre_trained_model
        num_convnext_features = self.convnext.head.fc.in_features
        self.convnext.head.fc = nn.Identity()  # Remove final FC layer to get feature vector

        # LSTM for sequence modeling
        self.lstm = nn.LSTM(input_size=num_convnext_features, hidden_size=lstm_hidden_size, num_layers=lstm_num_layers, batch_first=True)
    ...
    def forward(self, x):
        ...
        # Process each frame through ConvNeXt
        convnext_features = []
        for t in range(seq_len):
            frame = x[:, t, :, :, :]  # Extract the t-th frame from each sequence
            features = self.convnext(frame)  # Pass each frame through ConvNeXt
            convnext_features.append(features)

        # Stack the features along the time dimension
        convnext_features = torch.stack(convnext_features, dim=1)  # Shape: (batch_size, num_frames, num_convnext_features)
        ...
        lstm_out, (hn, cn) = self.lstm(convnext_features, (h0, c0))  # Shape of lstm_out: (batch_size, num_frames, lstm_hidden_size)
...
val_loader = DataLoader(val_set, batch_size=16, shuffle=True, num_workers=4, pin_memory=True)

# Set device (GPU if available, else CPU)
device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")

# Initialize the model
pre_trained_model = timm.create_model('convnext_base', pretrained=True)
model = ConvNeXt_LSTM_Model(pre_trained_model=pre_trained_model, lstm_hidden_size=512, lstm_num_layers=1, num_classes=len(cols_to_predict_value), num_frames=12)
